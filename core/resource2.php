...
class cli_formatter{
     ...
     * @param string|false $colour The color of the text.
     * @param string|false $background The background color for the text.
     * @param boolean $newline Weather to add a newline character at the end of the string.
     * @param string|false $attributes A comma seperated list with no spaces of style attributes.
     * @return string The formatted string.
     */
    public static function formatLine(string $string, string|false $colour=false, string|false $background=false, bool $newline=true, string|false $attributes=false):string{
...
class cmd{
    ...
    public static function newWindow(string $command, bool $keepOpen=false):void{
        mklog(1, 'Starting process with command ' . $command);
        $env = getEnvironment();
        if($env['windows']){
            pclose(popen('start cmd.exe /' . ($keepOpen ? "k" : "c") . ' "' . escapeshellcmd($command) . '"','r'));
        }
        else{
            if(!$env['desktop']){
                mklog(3, "You cannot run newWindow in this environment");
                return;
            }

            $terminal = null;

            foreach(['gnome-terminal', 'xterm', 'konsole', 'xfce4-terminal', 'lxterminal'] as $term){
                if(shell_exec("which $term 2>/dev/null")){
                    $terminal = $term;
                    break;
                }
            }
            ...
            if($terminal){
                $escapedCwd = escapeshellarg(getcwd());

                // Build the inner bash command
                $innerCmd = $keepOpen ? "$command; bash" : $command;

                $escapedCmd = escapeshellarg($innerCmd);

                $launch = match($terminal){
                    'gnome-terminal' => "gnome-terminal --working-directory=$escapedCwd -- bash -c $escapedCmd",
                    'konsole'        => "konsole --workdir $escapedCwd -e bash -c $escapedCmd",
                    'xfce4-terminal' => "xfce4-terminal --working-directory=$escapedCwd -e bash -c $escapedCmd",
                    'lxterminal'     => "lxterminal --working-directory=$escapedCwd -e bash -c $escapedCmd",
                    'xterm'          => "cd $escapedCwd && xterm -e bash -c $escapedCmd",
                    default          => null,
                };
            ...
            mklog(3, "Failed to find terminal program");
        }
    }
    /**
    ...
    /**
     * Returns the name of the start script.
     *
     * @return string The name of the start script.
     */
    public static function startScript():string{
        return $_SERVER['LAUNCHER_SCRIPT'] ?? 'start.' . (PHP_OS_FAMILY === 'Linux' ? 'sh' : 'bat');
    }
}
/**
...
class files{
    ...
    public static function validatePath(string $path, bool $addquotes=false):string{
        $path = PHP_OS_FAMILY === 'Linux' ? str_replace("\\","/",$path) : str_replace("/","\\",$path);
...
function extension_enable(string $extension):bool{
    return extensions::setEnabled($extension, true);
}
/**
 * Checks weather an extension is loaded, and enables it if not allready loaded.
 *
...
function extension_ensure(string $extension):bool{
    return extensions::ensure($extension);
}